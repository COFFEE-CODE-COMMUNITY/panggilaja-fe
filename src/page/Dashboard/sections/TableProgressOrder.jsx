...
const TableProgressOrder = () => {
    ...
    // Close modal when success is detected (handled by the effect above effectively, but let's be explicit for UI state)
    useEffect(() => {
    ...
    const dashboardDataFilterProgress = dashboardData?.filter((data) => data?.status === 'in_progress' || data?.status === 'proses')

    const { searchQuery } = useOutletContext() || { searchQuery: "" };

    const filteredData = dashboardDataFilterProgress?.filter(item => {
                ...
                <div className="space-y-4 mt-2">
                    {dashboardDataFilterProgress?.length === 0 && (
                        <div className="p-8 text-center bg-gray-50 rounded-lg border border-dashed border-gray-300">
                            ...
                            <p className="text-sm text-gray-500 max-w-md mx-auto">Pesanan yang sedang dalam proses akan muncul di sini. Mulai terima pesanan pertama Anda!</p>
                        </div>
                    )}
                            ...
                            <div className="p-6 grid grid-cols-12 gap-4 items-center">
                                ...
                                <div className="col-span-2">
                                    <span className={`text-xs font-semibold px-2 py-1 rounded inline-block ${order?.status === 'selesai' || order?.status === 'completed' ? 'bg-green-100 text-green-700' :
                                        order?.status === 'in_progress' || order?.status === 'proses' ? 'bg-yellow-100 text-yellow-700' :
                                            'bg-gray-100 text-gray-700'
                                        }`}>
                                        {order?.status === 'in_progress' ? 'Proses' : order?.status}
                                    </span>
                                </div>
                    ...
                    <div key={order?.order_id} className="bg-white border border-gray-300 rounded-lg p-4 shadow-sm">
                        ...
                        <div className="mt-3 pt-3 border-t border-gray-200 flex gap-2">
                            ...
                            <button
                                onClick={() => handleOrderanSelesai(order?.order_id)}
                                disabled={order?.status === 'completed' || order?.status === 'selesai'}
                                className="flex-1 py-2 rounded-lg border border-primary text-primary bg-white text-sm font-medium hover:bg-primary/5 transition-colors disabled:opacity-50 disabled:cursor-not-allowed disabled:border-gray-300 disabled:text-gray-400"
                            >
                                Orderan Selesai
                            </button>
                        </div>
                ...
                <div className="p-6">
                    ...
                    <div className="flex justify-end gap-3">
                        ...
                        <button
                            ...
                            className="cursor-pointer px-4 py-2 rounded-lg bg-primary text-white hover:bg-primary/90 transition-colors flex items-center gap-2"
                            disabled={updateStatus === 'loading'}
                        >
